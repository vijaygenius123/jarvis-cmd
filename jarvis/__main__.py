...
def task(status, task_id):
    ...
    click.echo(str(task['id']) + '\t' + task['_links']
               ['status']['title'] + '\t' + task['subject'])
...
def tasks():
    ...
    # Request For Workpackage
    resp = requests.get(
        base_url + '/api/v3/work_packages?pageSize=25&filters=[{ "assigneeOrGroup": { "operator": "=", "values": "me" }}]&sortBy=[["id", "asc"]]', auth=HTTPBasicAuth('apikey', api_key))
    json_resp = loads(resp.text)
